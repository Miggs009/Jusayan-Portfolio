...
function fetchAndDisplayBookings() {
    ...
    db.collection("bookings").get().then((querySnapshot) => {
        ...
        document.getElementById('todayBookingsContainer').innerHTML = '';
        document.getElementById('tomorrowBookingsContainer').innerHTML = '';
        ...
        querySnapshot.forEach((doc) => {
            ...
            // Parse the date or dates
            const dates = bookingData.date.split(',').map(date => new Date(date.trim()));

            // Check if today or tomorrow is included in the booking dates
            if (dates.some(date => isSameDate(date, today))) {
                displayBooking('todayBookingsContainer', bookingData, doc); // Pass doc as argument
            } else if (dates.some(date => isSameDate(date, tomorrow))) {
                displayBooking('tomorrowBookingsContainer', bookingData, doc); // Pass doc as argument
            }
        });
    }).catch((error) => {
        console.error("Error fetching bookings: ", error); // Log error to console
...
function displayBooking(containerId, bookingData, doc) {
    ...
    // Populate the booking item with relevant data
...
window.onload = fetchAndDisplayBookings;
...
if (user) {
    ...
    db.collection("users").doc(user.uid).get().then((doc) => {
        if (doc.exists && (doc.data().role === "admin" || doc.data().role === "manager")) {
            ...
            fetchAndDisplayBookings(); // Or any other function to initialize the dashboard
        } else {
